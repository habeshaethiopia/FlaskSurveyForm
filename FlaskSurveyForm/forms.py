...
class TrainingForm(FlaskForm):
    ...
    training_description = TextAreaField(
        "Training Description",
        validators=[DataRequired()],
        description="Briefly describe the training delivered",
    )
    ...
    # Trainee Days
    trainee_days = FloatField(
        "Trainee Days",
        validators=[Optional(), NumberRange(min=0)],
        description="How much time the trainees spent at the training, in working days",
    )
    ...
    other_cost_description = TextAreaField(
        "Other Expenses Description",
        validators=[Optional()],
        description="Please describe the other expenses",
    )
    concur_claim = StringField("Concur Claim Number")

    # Updated attendee field - now optional
    attendee_emails = TextAreaField(
        "Attendee/Trainee Emails (comma/space separated)", validators=[Optional()]
    )
    ...
    def validate_concur_claim(self, field):
        ...
        has_expenses = any(
            [
                self.travel_cost.data,
                self.food_cost.data,
                self.materials_cost.data,
    ...
    def validate_other_cost_description(self, field):
        """Validate that other cost description is provided when other cost is entered"""
        if self.other_cost.data and not field.data:
            raise ValidationError("Description is required for other expenses.")
    ...
    def prepare_form_data(self):
        ...
        return {
            ...
            "other_cost_description": self.other_cost_description.data,
