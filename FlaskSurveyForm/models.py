...
def create_tables():
    ...
    cursor.execute(
        ...
        CREATE TABLE IF NOT EXISTS attachments (
            ...
            FOREIGN KEY (training_id) REFERENCES training_forms (id)
        )
    """
    )
    conn.commit()
...
def insert_training_form(form_data):
    ...
    cursor.execute(
        ...
        INSERT INTO training_forms (
            training_type, training_description, trainer_name, supplier_name, location_type,
            location_details, start_date, end_date,
            trainer_days, trainee_days, trainees_data, approved, concur_claim, travel_cost,
            food_cost, materials_cost, other_cost, other_cost_description
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """,
        (
            ...
            form_data.get("other_cost_description"),
        ),
    )
...
def get_training_form(form_id):
    ...
    if row:
        ...
        return {
            ...
            "trainee_days": row_dict["trainee_days"],
            ...
            "other_cost_description": row_dict.get("other_cost_description"),
        ...
        forms.append(
            {
                "id": row[0],
                "training_type": row[1],
                "training_description": row[2],
                "trainer_name": row[3],
                "supplier_name": row[4],
                "location_type": row[5],
                "location_details": row[6],
                "start_date": row[7],
                "end_date": row[8],
                "trainer_days": row[9],
                "trainee_days": row[10],
                "trainees_data": row[11],
