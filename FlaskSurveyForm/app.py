...
# Set up the database (only create tables if they don't exist)
...
def get_unique_filename(filename, folder_path):
...
def submit():
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            try:
                ...
                form_folder = str(uuid.uuid4())
                folder_path = os.path.join(app.config["UPLOAD_FOLDER"], form_folder)
                ...
                if "attachments" in request.form:
                    ...
                    for i, filename in enumerate(request.form.getlist("attachments[]")):
                ...
                # Insert form data
                insert_training_form(form_data)
    ...
    return render_template("index.html", form=TrainingForm())
...
def edit_form(form_id):
    ...
    form = TrainingForm()
    existing_attachments = []  # Initialize the variable
    ...
    if request.method == "GET":
        ...
        if form_data:
            ...
            form.training_description.data = form_data["training_description"]
            ...
            form.trainee_days.data = form_data["trainee_days"]
            ...
            form.other_cost_description.data = form_data.get("other_cost_description")
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT filename, description FROM attachments WHERE training_id = ?",
                (form_id,),
            )
    ...
    if form.validate_on_submit():
        try:
            ...
            if "attachments" in request.files:
                ...
                # Create folder for this form's attachments
                folder_name = f"form_{form_id}"
                ...
                for i, file in enumerate(files):
                    if (
                        ...
                        unique_filename = get_unique_filename(filename, folder_path)
                        file_path = os.path.join(folder_path, unique_filename)
                        file.save(file_path)

                        # Save attachment record to database
                        description = descriptions[i] if i < len(descriptions) else None
                        save_attachment(form_id, unique_filename, description)
...
def save_attachment(form_id, filename, description=None):
    """Save attachment record to database"""
    conn = get_db()
    cursor = conn.cursor()
...
def upload_attachment():
    ...
    if file and allowed_file(file.filename):
        # Create a unique folder for this form submission
        form_folder = str(uuid.uuid4())
        folder_path = os.path.join(app.config["UPLOAD_FOLDER"], form_folder)
        os.makedirs(folder_path, exist_ok=True)
        ...
        return jsonify(
            {"success": True, "filename": unique_filename, "folder": form_folder}
        )

    return jsonify({"success": False, "error": "File type not allowed"}), 400


def insert_attachment(training_id, filename, description=None):
    ...
    try:
        cursor.execute(
            "INSERT INTO attachments (training_id, filename, description) VALUES (?, ?, ?)",
            (training_id, filename, description),
        )
        conn.commit()
    except Exception as e:
        conn.rollback()
